    ...
    <div class="container mt-5">
        ...
        <form action="{{ url('/donante/' . $donante->id) }}" method="POST" class="p-4 border rounded shadow">
            ...
            <div class="mb-3">
                ...
                <input type="text" id="nombre" name="nombre" class="form-control" value="{{ $donante->nombre }}" required>
            </div>

            <div class="mb-3">
                <label for="apellido" class="form-label">Apellido:</label>
                <input type="text" id="apellido" name="apellido" class="form-control" value="{{ $donante->apellido }}" required>
            </div>

            <div class="mb-3">
                <label for="cedula" class="form-label">Cédula:</label>
                <input type="number" id="cedula" name="cedula" class="form-control" value="{{ $donante->cedula }}" required>
            </div>

            <div class="mb-3">
                <label for="telefono" class="form-label">Teléfono:</label>
                <input type="text" id="telefono" name="telefono" class="form-control" value="{{ $donante->telefono }}" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <select id="sexo" name="sexo" class="form-control" required>
                    @foreach (App\Enums\Sexo::cases() as $tipo)
                        <option value="{{ $tipo->value }}" {{ $donante->sexo === $tipo->value ? 'selected' : '' }}>
                            {{ $tipo->value }}
                        </option>
                    @endforeach
                </select>
